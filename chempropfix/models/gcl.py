...
class E_GCL(nn.Module):
    ...
    def edge_model(self, source, target, radial, edge_attr):
        if edge_attr is None:  # Unused.
            out = torch.cat([source, target, radial], dim=1)  # 在列上拼接
        else:
            out = torch.cat([source, target, radial, edge_attr], dim=1)
        # edge_mlp
        out = self.edge_mlp(out)
        if self.attention:
            att_val = self.att_mlp(out)  # 注意力得分
            out = out * att_val
        return out   # mij [*,source+target+radial]
    ...
    def coord2radial(self, edge_index, coord):
        row, col = edge_index   
        # coord_diff：坐标差  difference：差距 向量
...
def unsorted_segment_sum(data, segment_ids, num_segments):
    ...
    result = data.new_full(result_shape, 0)  # Init empty result tensor.  #用0填充形状为result_shape的张量
